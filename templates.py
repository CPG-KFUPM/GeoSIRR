...
## Syn-rift sedimentary basin fill
...
- Each wedge rests directly on the tilted basement top, is thickest beside the normal fault on its left, and thins progressively to the right until it pinches out onto the crest of the tilted basement block. The basement-sediment contact must rise gently rightward, consistent with block rotation.
...
- Place the approximate rightward pinchouts of `basin_fill^basin_1` through `basin_fill^basin_5` near x = 16, 26, 35, 43, and 49.5 km, respectively. Use at least one intermediate shaping vertex on both the top and basal contact of each wedge so that the sediment surfaces are gently irregular rather than simple triangles.
...
- Visual priority, in order: (1) exactly six smooth, linked east-dipping listric faults; (2) exactly two west-dipping antithetic faults on the right; (3) six wedge-shaped sediment fills thickening leftward toward their bounding faults and pinching out on tilted basement crests; (4) the repeated tilted-block sawtooth topography; (5) the high left and right plateaus and lower central rift basin; (6) complete rectangular coverage by air, basin fill, and fault-split basement.
""",
